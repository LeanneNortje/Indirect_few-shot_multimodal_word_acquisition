            ...
            for w in episode['queries']:
                ...
                if lookup in alignments:
                    if w in alignments[lookup]:
                        ...
                        q_image = [support_set_images[indices[a], :, :].unsqueeze(0) for a in range(top_N)]
                        q_image = torch.cat(q_image, dim=0).mean(dim=0)

                        scores = attention.one_to_many_score_images(m_images, q_image.unsqueeze(0))
