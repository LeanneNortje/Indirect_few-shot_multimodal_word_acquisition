...
support_set = np.load('support_set/support_set_50.npz', allow_pickle=True)['support_set'].item()
top_N = 5
            ...
            for w in episode['queries']:
                ...
                if lookup in alignments:
                    if w in alignments[lookup]:
                        ...
                        indices = torch.argsort(scores, descending=True)[0:5]
                        pred_labels = [support_set_labels[indices[a]] for a in range(top_N)]
                        ...
                        scores = attention.one_to_many_score_images(m_images, support_set_images[indices[ind], :, :].unsqueeze(0))
