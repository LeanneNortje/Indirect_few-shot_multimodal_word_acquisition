...
with torch.no_grad():
    ...
    for wav_name in tqdm(support_set):
        wav, im, spkr, _, _, w = support_set[wav_name]

        this_english_audio_feat, this_english_nframes = LoadAudio(image_base / 'SpokenCOCO' / wav, (0, target_length), audio_conf)
        ...
        support_set_audio.append(this_english_output)
        support_set_frames.append(this_english_nframes)
        ...
        support_set_images.append(this_image_output)
        support_set_labels.append(w)

    support_set_images = torch.cat(support_set_images, axis=0)
    support_set_audio = torch.cat(support_set_audio, axis=0)
    print(support_set_audio.size())
    support_set_frames = torch.cat(support_set_frames, axis=0)
            ...
            for w in episode['queries']:
                ...
                if lookup in alignments:
                    if w in alignments[lookup]:
                        ...
                        # scores = attention.one_to_many_score(m_images, query, n_frames).squeeze()
                        scores = attention.one_to_many_score_audio(support_set_audio, query, n_frames).squeeze()
                        scores = softmax(scores)
                        den = torch.exp(-1*scores/temperature).sum(dim=-1)
                        scores = torch.exp(-1*scores/temperature) / den 
                        # ind = torch.argsort(scores)[0:top]
                        # pred_labels = [support_set_labels[ind[a]] for a in range(top)]
                        # pred_class = max(set(pred_labels), key=pred_labels.count)

                        # count = pred_labels.count(pred_class)
                        # if count == 1:
                        #     ind = 0
                        #     pred_class = pred_labels[0]
                        # else:
                        #     for a in range(top):
                        #         if pred_class == pred_labels[a]:
                        #             ind = a
                        #             break
                        # q = [support_set_images[ind[a], :, :].unsqueeze(0) for a in range(top)]
                        # q = torch.cat(q, dim=0)
                        # q = q.mean(dim=0).unsqueeze(0)
                        
                        ind = torch.argmax(scores).item()

                        scores = attention.one_to_many_score_images(m_images, support_set_images[ind, :, :].unsqueeze(0))
                        den = torch.exp(-1*scores/temperature).sum(dim=-1)
                        scores = torch.exp(-1*scores/temperature) / den 
                        ind = torch.argmax(scores).item()
