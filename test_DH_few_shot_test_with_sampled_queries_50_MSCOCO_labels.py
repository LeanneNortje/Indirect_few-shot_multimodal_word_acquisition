...
with torch.no_grad():
    ...
    for im in tqdm(episodes['matching_set']):
        ...
        if len(episodes['matching_set'][im]) == 0: 
            matching_set_labels.append(' ')
        else:
    ...
    for i_test in range(5):
        ...
        for w in queries:
            ...
            indices = torch.argsort(scores, descending=True)[0]
            scores = attention.one_to_many_score_images(matching_set_images, support_set_images[indices, :, :].unsqueeze(0))
            indices = torch.argsort(scores, descending=True).squeeze()[0: counting[w]]
